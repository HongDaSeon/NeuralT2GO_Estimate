...
class Craft:
    ...
    def simulate_via_pqrD(self, pqrD):
    ...
    def simulate_via_acc(self, acc, Satur=True, boundnum=99):
        if Satur:
            acc = Daseon.Vector3.cast(np.clip(acc.vec,-boundnum,boundnum))
        self.datt.x         = 0
        self.datt.y         = -(acc.z + gAcc*m.cos(self.att.y))/self.scavel
        self.datt.z         = acc.y/self.scavel/m.cos(self.att.y)
